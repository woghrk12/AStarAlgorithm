...
# 📚 결론 및 회고
이번 프로젝트를 통해 단순히 알고리즘을 빠르게 구현하는 것보다, 구조적으로 문제를 분석하고 해결책을 설계하는 능력의 중요성을 다시 한번 깨닫게 되었습니다.

### ✅ 결론 요약
- Region 단위로 맵을 분할하고, 계층적으로 경로 탐색을 수행한 결과, 노드 방문 수가 줄어들어 경로 탐색 비용이 감소
- 이로 인해 동시에 여러 유닛이 A\*를 수행하더라도 프레임 유지 가능
- Region 단위로 경로를 재활용하거나, **Region 수준 캐싱** 등의 추가 확장이 가능

### 💭 회고
- A\* 알고리즘은 단순한 휴리스틱 연산 이상의 개념이며 "불필요한 탐색 대상 얼마나 많은가"가 성능의 핵심 병목임을 깨달았습니다.
- 스타크래프트처럼 상용 게임에서 쓰이는 알고리즘 구조를 직접 실험해볼 수 있었으며, 단순 구현이 아닌 게임 환경에 적합한 최적화 구조 설계 능력을 기를 수 있었습니다.
- 또한, Unity 내에서의 충돌 체크, 좌표 변환, 객체 간 상호작용 등도 함께 고려한 설계 경험을 통해 알고리즘 자체와 게임 엔진 특성 간의 연결을 이해하게 된 점도 큰 수확이었습니다.
